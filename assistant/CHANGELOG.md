...
- **Fix**: Function tools now work with the gpt-5.4/5.5/5.6 family on `/v1/chat/completions`. These models apply a default reasoning effort even when the param is omitted, so requests carrying tools hit `400 Function tools with reasoning_effort are not supported ... set reasoning_effort to 'none'`. We now explicitly send `reasoning_effort="none"` when tools are present instead of just dropping the param.
