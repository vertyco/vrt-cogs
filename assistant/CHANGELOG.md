...
- **New**: `[p]assistant codex status` shows which login a server is using (server, global, or host file), the plan, when it expires, and the models the plan allows. `[p]assistant set model` validates against that list while Codex is active and falls back to `gpt-5.5` for models the plan does not serve.
- **Note**: When no endpoint override is set, a Codex login is always tried first: the server's Codex login, then the global Codex login, then the host's `auth.json`. If the subscription request fails for any reason (usage limit, expired login, outage) and an API key is configured, that message is answered with the API key instead. Without an API key the Codex error is shown as before. Any endpoint override (server or global) disables Codex for that server. Embeddings and image generation are not served by the subscription and still use the API key or endpoint override. Temperature, seed, and penalty settings do not apply on the subscription path (the Codex backend does not accept them).
- **New**: Added `gpt-6-astra` to the model list (1.05M context).
- **New**: Key check. When an endpoint override is removed (`[p]assistant api guild endpoint none`, `api guild clear`, or `api globalendpoint` with no argument) or an API key is set, the reply now warns if the key left behind will not work against OpenAI directly: a key that looks like an OpenRouter key, a key OpenAI rejects, no key at all, or an embedding model that is not an OpenAI model. The override is still removed; nothing is blocked. The same check shows as a `Key Check` line in `[p]assistant view` and in `[p]assistant codex status`, and is skipped while an endpoint override is active.
...
- **New**: The GPT-5.4/5.5/5.6 family now honors the configured `reasoning_effort` while function tools are active. Chat Completions cannot combine reasoning with tools for these models, so calls that need both are routed through the Responses API (`/v1/responses`) and the result is adapted back into the normal response shape, leaving conversation storage, tool-call parsing, and usage/cache accounting unchanged. Non-tool turns, other models, and `reasoning_effort: none` stay on Chat Completions. Previously the reasoning setting was dropped whenever tools were present.
- **Fix**: The GPT-5.6 family no longer errors out when function tools are registered (`Function tools with reasoning_effort are not supported ... use /v1/responses or set reasoning_effort to 'none'`). With reasoning enabled the call now goes through the Responses API (above); with `reasoning_effort: none` it stays on Chat Completions with the effort set to `none` explicitly, which 5.6 requires alongside tools.
