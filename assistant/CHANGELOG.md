...
- **New**: The GPT-5.4/5.5/5.6 family now honors the configured `reasoning_effort` **while function tools are active**. Chat Completions cannot combine reasoning with tools for these models, so calls that need both are transparently routed through the Responses API (`/v1/responses`) and the result is adapted back into the normal response shape — conversation storage, tool-call parsing, and usage/cache accounting are unchanged. Non-tool turns, other models, and `reasoning_effort: none` stay on Chat Completions. Previously the reasoning setting was silently dropped whenever tools were present.
