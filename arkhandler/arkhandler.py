...
async def watchdog():
    ...
    if "ShooterGame.exe" in (p.name() for p in psutil.process_iter()):
        ...
        if not UPDATING:
            ...
            await send_hook("Server Down", "Beginning reboot sequence...", 16739584)
            ...
            await asyncio.sleep(0.5)
            print(Fore.GREEN + "Boot macro finished")
            log.info(f"Boot macro finished")
            await send_hook("Booting", "Loading server files...", 19357)
            await asyncio.sleep(8)
            print(Fore.MAGENTA + "Attempting to stop LicenseManager")
            log.info(f"Attempting to stop LicenseManager")
            os.system("net stop LicenseManager")
            await asyncio.sleep(35)
            await send_hook("Reboot Complete", "Server should be back online.", 65314)
            log.info(f"Reboot Macro Completed")
