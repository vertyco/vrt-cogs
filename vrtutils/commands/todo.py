...
async def mock_edit_message(interaction: discord.Interaction, message: discord.Message):
    ...
    bots_message = message.author.id == bot.user.id
    if not bots_message and not message.webhook_id:
        ...
        if not await bot.is_admin(member):
    ...
    if bots_message:
        ...
        # User is human, we'll need to mock with a webhook
