...
STANCE_INFO: dict[MovementStance, dict] = {
...
class EditorTacticsView(ui.LayoutView):
    ...
    def _build_layout(self):
        ...
        stance_data = STANCE_INFO.get(orders.movement_stance, {"emoji": "?", "title": "Unknown", "desc": "Unknown"})
        stance_icon, stance_desc = stance_data["emoji"], stance_data["desc"]
...
class EditorTacticsControlsRow(ui.ActionRow["EditorTacticsView"]):
    ...
    @ui.button(label="Reset to Default", style=discord.ButtonStyle.secondary, row=2)
    ...
    async def _build_layout(self):
        ...
        if not self.player.owned_chassis:
            ...
            for idx, chassis in enumerate(display_bots):
                ...
                tactics_str = (
                    f"{orders.movement_stance.value.title()} | {orders.target_priority.value.replace('_', ' ').title()}"
                )
