...
- 30 second poll loop with join/leave and server online/offline log embeds.
...
- `[p]paltools findplayer` shows IP history and shared-IP overlaps only when `logips` is on, the person asking is a mod or higher, and the channel is not readable by `@everyone`.
